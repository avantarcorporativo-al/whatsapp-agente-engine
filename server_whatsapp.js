/**
 * ==============================================================================
 * AGENTE UNIVERSAL ENGINE - SERVER WHATSAPP (CÓDIGO 100% CORREGIDO Y FUNCIONAL)
 * ==============================================================================
 * Stack: Node.js, Express, Socket.io, @whiskeysockets/baileys, Gemini REST API v1beta
 * ==============================================================================
 */
...
// 4. NORMALIZACIÓN DE JID DE WHATSAPP (@lid a @s.whatsapp.net)
...
// 5. LLAMADA HTTPS A LA API DE GEMINI (CON SOPORTE NATIVO V1BETA Y REINTENTOS)
async function llamarEndpointGemini(modelo, apiKey, payloadJSON) {
            ...
            res.on('end', () => {
                if (res.statusCode === 200) {
                    try {
                        ...
                        if (text && text.trim()) {
                            return resolve({ success: true, text: text.trim() });
                        }
                    } catch (e) {
                        console.error("❌ Error parseando JSON de Gemini:", e.message);
                    }
                }
                console.error(`❌ Gemini API Error (${modelo}) HTTP ${res.statusCode}: ${body.substring(0, 200)}`);
                resolve({ success: false, statusCode: res.statusCode, body });
            });
        });

        req.on('error', (err) => {
            console.error("❌ Error de red HTTPS con Gemini:", err.message);
...
async function consultarGeminiHTTPS(promptUsuario, historial = []) {
    ...
    if (!apiKey) {
        console.error("❌ Error: No hay una API Key de Gemini configurada.");
        return null;
    }

    const instrucciones = configActual.instruccionesUniversales || "Eres un Agente Virtual atento y servicial.";

    // Construcción limpia y con alternancia estricta de turnos (user / model)
    ...
    if (Array.isArray(historial) && historial.length > 0) {
        ...
        historial.forEach(item => {
            ...
            const texto = (item.texto || item.text || "").trim();
            if (texto && role !== lastRole) {
    ...
    // Agregar el mensaje actual del usuario asegurando no duplicar turno 'user'
    ...
    // Payload con la estructura nativa v1beta de Gemini
    const bodyObj = {
        system_instruction: {
    ...
    // Lista de modelos a intentar en cascada por si alguno falla o no tiene cuota
    const modelosATratar = [
        configActual.modeloGemini || "gemini-2.5-flash",
        "gemini-2.0-flash",
        "gemini-1.5-flash"
    ];

    const modelosUnicos = [...new Set(modelosATratar)];

    for (const mod of modelosUnicos) {
        const res = await llamarEndpointGemini(mod, apiKey, payloadJSON);
...
async function procesarRespuestaIA(remitenteJID, textoCliente) {
    try {
        ...
        if (respuestaIA && sock) {
            ...
            console.log(`🤖 [AGENTE IA RESPONDIÓ A ${remitenteJID}]: ${respuestaIA}`);
            ...
            console.warn(`⚠️ No se pudo enviar mensaje a ${remitenteJID}. Gemini devolvió respuesta vacía o la API Key requiere actualización.`);
        }
    } catch (err) {
...
// 7. INICIALIZACIÓN COMPLETA DE BAILEYS WHATSAPP
    ...
    try {
        ...
        sock.ev.on('connection.update', async (update) => {
            ...
            if (connection === 'close') {
                ...
                if (isLoggedOut) {
                    console.log('⚠️ Sesión desvinculada desde el celular. Limpiando credenciales...');
        ...
        sock.ev.on('messages.upsert', async (m) => {
            try {
                ...
                console.log(`💬 [WHATSAPP RECIBIDO EN SERVIDOR]: ${textoEntrante} (JID: ${jidLimpio})`);
...
// 8. ENDPOINTS REST API DE EXPRESS
